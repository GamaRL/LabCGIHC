...
std::string Shader::ReadFile(const char* fileLocation)
{
	...
	{
		std::cout << "Falló en leer el archivo " << fileLocation << std::endl;
		return "";
	}
	std::string line = "";
...
void Shader::CompileShader(const char* vertexCode, const char* fragmentCode)
{
	...
	if (!shaderID)
	{
		std::cout << "Error creando el shader" << std::endl;
		return;
	}
	...
	if (!result)
	{
		...
		std::cout << "El error al linkear es: " << eLog << std::endl;
		return;
	}
	...
	{
		glGetProgramInfoLog(shaderID, sizeof(eLog), NULL, eLog);
		std::cout << "El error al validar es: " << eLog << std::endl;
		return;
	}
...

{
	...
	GLint codeLength[1];
	codeLength[0] = std::string(shaderCode).size();//longitud del texto
	...
	{
		glGetProgramInfoLog(shaderID, sizeof(eLog), NULL, eLog);
		std::cout << "El error al compilar el shader " << shaderType << " es: " << eLog << std::endl;
		return;
	}
